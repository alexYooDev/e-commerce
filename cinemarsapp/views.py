...
def add_order():  
    ...
    if dvd_id is not None and order is not None:
        ...
        try:
            ...
            flash("You already have this DVD in your basket!")
...
def empty_order():
    if "order_id" in session:
        ...
        flash("There is no order to empty out!")
        return redirect(request.referrer)
    return redirect(url_for("main.add_order"))
...
def checkout_order():
    ...
    if "order_id" in session:
        order = db.session.scalar(db.select(Order).where(Order.id == session["order_id"]))
        if not order.dvds:
            flash("You haven't added any items in your basket!")
            return redirect(url_for("main.add_order"))
...
def get_dvds(category):
  ...
  try:
    if category == "movie":
        dvds = db.session.scalars(db.select(DVD).where(DVD.category == "movie"))
    if category == "series":
        dvds = db.session.scalars(db.select(DVD).where(DVD.category == "series"))  
    if category == "intl":
        dvds = db.session.scalars(db.select(DVD).where(DVD.original_title != DVD.title))
  except Exception as e:
      flash("No DVDs Found!")
...
def get_dvds_by_genres(genre_id):
    ...
    try:
        dvds = db.session.query(DVD).join(dvd_genre_association).join(Genre).filter(Genre.id == genre_id).all()
    except Exception as e:
        flash("No DVDs Found!")
        return render_template("error.html", error=repr(e))
    return render_template("dvds.html", dvds=dvds)
